...
import 'package:grocerry/models/customer.dart';
import 'package:grocerry/models/login_model.dart';
...
Future<List<CategoryModel>> getCategories() async{
  ...
  if(response.statusCode == 200 ){
    data = (response.data as List).map((i) => CategoryModel.fromJson(i)).toList();
  }
  }on DioError catch(e) {
    print(e.response);
  }
...
Future<List<ProductModel>> getProducts(String tagId) async {
  ...
  try{
    ...
    if(response.statusCode == 200 ){
    data = (response.data as List).map((i) => ProductModel.fromJson(i)).toList();
  }
